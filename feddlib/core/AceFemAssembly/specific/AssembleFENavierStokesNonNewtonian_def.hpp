...
namespace FEDD
{
    ...
    ******************************************************************************* */
    ...
    AssembleFENavierStokesNonNewtonian<SC, LO, GO, NO>::AssembleFENavierStokesNonNewtonian(int flag, vec2D_dbl_Type nodesRefConfig, ParameterListPtr_Type params, tuple_disk_vec_ptr_Type tuple) : AssembleFENavierStokes<SC, LO, GO, NO>(flag, nodesRefConfig, params, tuple)
    {
    ...
    void AssembleFENavierStokesNonNewtonian<SC, LO, GO, NO>::assemblyNeumannBoundaryTerm(SmallMatrixPtr_Type &elementMatrix) //, dblVecPtr normalVector)
    {
        ...
        Helper::getPhi(phi, QuadW, QuadPts, dim, FEType); // This should be zero for the basisfunction not laying on the line/ surface
