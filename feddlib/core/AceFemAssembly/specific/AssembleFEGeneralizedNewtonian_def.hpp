...
namespace FEDD
{
    ...
    */
    ...
    /* Based on the current solution (velocity, pressure etc.) compute pressure mass matrix \int \tilde{eta} psi_i psi_j dx
    where \tilde{eta} is the element averaged viscosity depending on shear rate at the quadrature points
    */
    template <class SC, class LO, class GO, class NO>
    void AssembleFEGeneralizedNewtonian<SC, LO, GO, NO>::assembleViscosityScaledPressureMassMatrix()
    {
        ...
        TEUCHOS_TEST_FOR_EXCEPTION(dim == 1, std::logic_error, "assembleEtaScaledMassMatrix Not implemented for dim=1");
        ...
        for (UN i=0; i < phi->at(0).size(); i++) 
        {
            for (UN j=0; j < phi->at(0).size(); j++) 
            {
                ...
                value[0] *= absDetB;
				(*elementMatrix)[i][j] +=  (-1.0/(1.0*viscosity_element_avg))*value[0];
            }
        }
