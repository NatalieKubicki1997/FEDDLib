...
AssembleFENavierStokes<SC,LO,GO,NO>(flag, nodesRefConfig, params,tuple)
{
	...
	this->dofsElementVelocity_ = dofsVelocity_*this->numNodesVelocity_;
	this->dofsElementPressure_  = dofsPressure_*this->numNodesPressure_;	
...
void AssembleFEStokes<SC,LO,GO,NO>::assembleJacobian() {
    ...
    //	this->jacobian_->add((*elementMatrixW),(*this->jacobian_));  // int add(SmallMatrix<T> &bMat, SmallMatrix<T> &cMat); //this+B=C elementMatrix + this->constantMatrix_;
    //}
}

template <class SC, class LO, class GO, class NO>
void AssembleFEStokes<SC,LO,GO,NO>::assembleFixedPoint() {
