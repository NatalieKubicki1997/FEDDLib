...
// This function is evaluated at each boundary node for each time step
void inflowPiecewiseLinear1DInterpolationFromData(double *x, double *res, double t, const double *parameters)
{
    // This function is used to interpolate inflow data from a file called inflowData.txt
    // The inflow data is stored in a file with the following format:
    // x1 y1
    // x2 y2
    // ...
    // xn yn
    // Important that it is space separated and no other characters like , !
    // The function reads the data and interpolates the inflow velocity at the current position here y
    // The inflow velocity is stored in the res array

    // @Important: The user has to ensure that the height is the same as in the mesh file    
    // Read the filename from the parameters array
    // The file should always read inflowData.txt
    std::string filename ="inflowData.txt";
    // Open the file
    ...
    double xValue, dataValue;
    while (file >> xValue >> dataValue)
    {
    ...
    // Close the file
    file.close();
    // Then we stored the data in xData and data accordingly and can access it via e.g. xData[i] and data[i]
    // Interpolate the inflow velocity at the current position x
    res[0] = 0.0;
    res[1] = 0.0;
    res[2] = 0.0;
    double x1, x2, y1, y2;
    ...
    {
        // Here we check where coordinate value is olcated
        if (xData[i] <= x[1] && x[1] <= xData[i + 1])
        {
            x1 = xData[i];
            x2 = xData[i + 1];
            y1 = data[i];
            y2 = data[i + 1];
            res[0] = y1 + (y2 - y1) / (x2 - x1) * (x[1] - x1);
            break;
        }
    }
    ...
    // Einlesen von Parameterwerten
    {
        ...
        // Inside we construct the mesh so define connectivities etc.
        {
            ...
            if (dim == 2)
            {
                ...
                bcFactory->addBC(inflowPiecewiseLinear1DInterpolationFromData, 4, 0, domainVelocity, "Dirichlet", dim, parameter_vec); // Analytical solution Power-Law 2D Poiseuille flow based on parameters and pressure gradient
            }
            else if (dim == 3)
            {
