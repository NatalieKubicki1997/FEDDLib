...
/*!
This is the main in order to generate the first training artficial training data as discussed with Dimitry
For that we needed:
1. material law: here we assumed an artifical material law which depends on shear rate and density
2. flow rate: in order to use same flow conditions we specify a flow rate using the flow rate we prescribe a constant inflow velocity
3. geometry: our geometry is a simple rectangular channel in simulation units
...
void constx2D(double *x, double *res, double t, const double *parameters)
{
    ...
    // Einlesen von Parameterwerten
    {
        ...
        // Inside we construct the mesh so define connectivities etc.
        {
            ...
            if (dim == 2)
            {
                ...
                bcFactory->addBC(constx2D, 4, 0, domainVelocity, "Dirichlet", dim, parameter_vec); // Analytical solution Power-Law 2D Poiseuille flow based on parameters and pressure gradient
            }
            else if (dim == 3)
            {
...

            {
                ...
                // So after the assemble Routine is called we read in an external variable field
                /*if (verbose)
                {
                cout << "###############################################################" << endl;
                cout << "##################### Read External field ####################" << endl;
                cout << "###############################################################" << endl;
                }
